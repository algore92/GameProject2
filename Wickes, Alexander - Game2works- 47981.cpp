/*Wickes, Alexander, 2nd game project, 12/1/13
Hl HO! CHERRY-O® gameboard with spinner, Now with Graphis
...
1. Each player chooses a tree and places 10 cherries on it.
2. The youngest player goes first. Play passes to the left.
Game Play
...
//keep the loop going until there is a winner
   ...
   //there is a winner
        {
        cout<<"Player "<<i<<" press any letter key and enter key to spin."<<endl;
...
// 7 if's for 7 positions on the spinner per game instructions above
         if (cherry[i]==1){
             string getFileContents (std::ifstream&); { ///Gets Art file contents
    system("CLS");  //clear the screen                    
    ifstream Reader ("cherry1.txt");             //Open file
...
{
system("CLS");
    ifstream Reader ("cherry2.txt");             //Open file
...
{
system("CLS");//clear the screen
    ifstream Reader ("cherry3.txt");             //Open file
...
{
system("CLS");
    ifstream Reader ("cherry4.txt");             //Open file
...
{
system("CLS");
    ifstream Reader ("bird5.txt");             //Open file
    ...
    Reader.close ();                           //Close file
              ...
              cout<<"A bird ate two cherries from your bucket."<<endl;
              cherry[i]=i-2;}
          else if (cherry[i]==6){  
             string getFileContents (std::ifstream&); //Gets Art file contents
{
system("CLS"); //clear screen
    ifstream Reader ("dog6.txt");      //Open file
    ...
    Reader.close ();                           //Close file
              ...
              cout<<"A dog ate two cherries from your bucket."<<endl;
              cherry[i]=i-2;
          } 
          else if (cherry[i]==7){  
              string getFileContents (std::ifstream&); {//Gets Art file contents
system("CLS"); //clear screen
    ifstream Reader ("cherrylost7.txt");             //Open file

    string Art = getFileContents (Reader);       //Get file
    
    cout << Art << endl;        //Print art work to the screen
